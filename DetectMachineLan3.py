...
""" Detector de direcciones MAC duplicadas o inexistentes en la Whitelist """
			...
			if str(v['status']['state']) == 'up':
				try:
					if c == 1:
						...
						mac_locate = z.find(str(v['addresses']['mac']))   
						z = z + "," + str(v['addresses']['mac'])

						if mac_locate = -1:
							mac_repeat = str(v['addresses']['mac'])
					...
					if mac_repeat != "":
						msg = 'Mac DUPLICATE ' +str(v['addresses']['mac'])+' Ip: '+str(v['addresses']['ipv4'])
						mac_repeat = ""
						if self.verbose:
							self.__consoleMessage(msg)
						if self.log:
